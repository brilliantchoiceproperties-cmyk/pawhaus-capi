...
- ✅ **Room Catalog v2 + Dog Difference Section (2026-02-16)** — Major room/pricing restructure:
   - Renamed `standard` room → `extended` ("Extended Room") and SPLIT it into two SKUs to measure hot-tub demand:
     - `extended` (no hot tub) — $623/wkday-1N base
     - `extended_ht` (with wood-fired hot tub) — $698/wkday-1N base, exactly $75/night premium
   - Bumped Monolith prices by $50/night across all stays ($898→$948 wkday/1N, $1478→$1578 wkday/2N, etc.)
   - Added `ROOM_ID_ALIASES = {"standard": "extended_ht"}` so historical bookings + URLs still resolve cleanly
   - Added `ROOM_DAILY_CAPS = {"extended_ht": 2}` — hard 409 when 3rd booking attempted on same check-in date (extended/petite/monolith remain uncapped)
   - New `DogDifferenceSection.jsx` on Landing with 4 cards (In your cabin / In your private yard / Across the property / Our service) covering all dog-specific amenities: dedicated dog bed, treat bag, food/water bowls preset, towel stash, designated potty corner with cedar waste station, ground-level auto-fill water bowl, shaded cedar dog cot, **coming soon: solar-heated outdoor shower + private cold rinse station per cabin**, 30 acres off-leash, water bowls everywhere, pup cups, named-greeting service, **Vetster 24/7 vet partnership**, 10–15min to daytime vet / 30min to 24-hr emergency hospital. Updated Landing copy (value props + house rules) to use new room names.
   - Pytest: 9/9 new tests pass (catalog, pricing, alias, cap blocks 3rd on extended_ht only). Total suite 41 tests passing. Frontend testing agent verified 100% — all 4 room cards render with correct prices + savings badges + perks line; DogDifferenceSection renders with all 4 cards + key text ('potty corner', 'auto-fill stainless water bowl', 'Vetster', 'pup cups').
